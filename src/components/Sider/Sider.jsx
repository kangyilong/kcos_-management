...
export default function SiderComponent(props) {
    ...
    return (<>
            <Sider width={200} style={{ background: '#fff' }}>
                <Menu mode="inline" defaultSelectedKeys={[selectedKeys]} defaultOpenKeys={[openKeys]} style={{ height: '100%', borderRight: 0 }}>
                    {Array.isArray(siderMenu) && siderMenu.map((item, index) => (<SubMenu key={`sub${index}`} title={<span>{item.name}</span>}>
                                {Array.isArray(item.children) && item.children.map((c_item, c_index) => (<Menu.Item key={`menu${index}${c_index}`}>
